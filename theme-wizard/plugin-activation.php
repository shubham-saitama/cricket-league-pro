	...
	class Vw_Premium_Theme_Plugin_Activation_Settings {
        ...
        public function cricket_league_pro_recommended_plugins_array($plugins){
            $plugins[] = array(
                ...
                'desc'     => esc_html__('it is recommended that you install the Contact Form 7 plugin to show contact form on pages', 'cricket-league-pro'),
            );
            // $plugins[] = array(
            //     'name'     => esc_html__('Ibtana - Ecommerce Product Addons', 'cricket-league-pro'),
        ...
        public function enqueue_scripts() {
            ...
            wp_localize_script('vw-theme-plugin-activation-script', 'cricket_league_pro_activate_plugin',
                array(
                    ...
                    'error' => esc_html__('Error', 'cricket-league-pro'),
        ...
        public function cricket_league_pro_get_plugin_buttion($slug, $name, $function) {
                ...
                if (!$is_installed) {
                    ...
                    $button_html        = sprintf('<a class="vw-theme-plugin-install install-now button-secondary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
                        ...
                        sprintf(esc_html__('Install %s Now', 'cricket-league-pro'), esc_html($name)),
                        esc_html($name),
                        esc_html__('Install & Activate', 'cricket-league-pro')
                    );
                    ...
                    $button_html = sprintf('<a class="vw-theme-plugin-activate activate-now button-primary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
                        ...
                        sprintf(esc_html__('Activate %s Now', 'cricket-league-pro'), esc_html($name)),
                        esc_html($name),
                        esc_html__('Activate', 'cricket-league-pro')
                    );
                } elseif ($is_activeted) {
                    $button_html = sprintf('<div class="action-link button disabled"><span class="dashicons dashicons-yes"></span> %s</div>', esc_html__('Active', 'cricket-league-pro'));
                    $is_done     = true;
                }
