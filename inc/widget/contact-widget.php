...
class wpb_widget extends WP_Widget
{
	function __construct()
	{
		parent::__construct(
			...
			__('Contact Us', 'cricket-league-pro'),
			// Widget description
			array('description' => __('Widget for About Us section', 'cricket-league-pro'), )
		);
	}
...
// This is where the action happens
		...
		<div class="about_me">
			...
			<p class="location">
				<?php if ($location != '') { ?>
					...
					<a class="contact-text" href="https://maps.google.com/?q=58250<?php echo esc_textarea($location); ?>"
						target="_blank">
	...
	public function form($instance)
	{
		...
		if (isset($instance['custom_title'])) {
			...
			esc_html($custom_title = __('New title', 'cricket-league-pro'));
		}
		//custom_content
		if (isset($instance['custom_content'])) {
			esc_html($custom_content = $instance['custom_content']);
		} else {
			esc_html($custom_content = __('Custom Content', 'cricket-league-pro'));
		}
		//Location
		if (isset($instance['location'])) {
			esc_html($location = $instance['location']);
		} else {
			esc_html($location = __('Location', 'cricket-league-pro'));
		}
		//Phone
		if (isset($instance['phone'])) {
			esc_html($phone = $instance['phone']);
		} else {
			esc_html($phone = __('Phone', 'cricket-league-pro'));
		}
		//Email
		if (isset($instance['email'])) {
			esc_html($email = $instance['email']);
		} else {
			esc_html($email = __('Email', 'cricket-league-pro'));
		}
		...
		if (isset($instance['time'])) {
			...
			esc_html($time = __('Time', 'cricket-league-pro'));
		}
		...
		<p>
			<label
				for="<?php echo esc_attr($this->get_field_id('custom_title')); ?>"><?php esc_html_e('Title:', 'cricket-league-pro'); ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id('custom_title')); ?>"
				name="<?php echo esc_attr($this->get_field_name('custom_title')); ?>" type="text"
				value="<?php echo esc_attr($custom_title); ?>" />
		</p>
		<!--Message field -->
		<p>
			<label
				for="<?php echo esc_attr($this->get_field_id('custom_content')); ?>"><?php esc_html_e('Content:', 'cricket-league-pro'); ?></label>
			<textarea class="widefat" id="<?php echo esc_attr($this->get_field_id('custom_content')); ?>"
				name="<?php echo esc_attr($this->get_field_name('custom_content')); ?>" type="text"
				value="<?php echo esc_attr($custom_content); ?>"><?php if (!empty($custom_content)) {
		...
		<p>
			<label
				for="<?php echo esc_attr($this->get_field_id('location')); ?>"><?php esc_html_e('Location:', 'cricket-league-pro'); ?></label>
			<textarea class="widefat" id="<?php echo esc_attr($this->get_field_id('location')); ?>"
				name="<?php echo esc_attr($this->get_field_name('location')); ?>" type="text"
				value="<?php echo esc_attr($location); ?>"><?php if (!empty($location)) {
		...
		<p>
			<label
				for="<?php echo esc_attr($this->get_field_id('phone')); ?>"><?php esc_html_e('Phone:', 'cricket-league-pro'); ?></label>
		...
		<p>
			<label
				for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php esc_html_e('Email:', 'cricket-league-pro'); ?></label>
		...
		<p>
			<label
				for="<?php echo esc_attr($this->get_field_id('time')); ?>"><?php esc_html_e('Time:', 'cricket-league-pro'); ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id('time')); ?>"
				name="<?php echo esc_attr($this->get_field_name('time')); ?>" type="text"
